...
public class SQLDatabaseHandler extends SQLiteOpenHelper {
    ...
    SQLcontact getContact(int id){
        ...
        if(cursor!=null)
            cursor.moveToFirst();
        SQLcontact contact=new SQLcontact(Integer.parseInt(cursor.getString(0)),cursor.getString(1),cursor.getString(2));
        return  contact;

    }
    //Getting All Contacts
    public List<SQLcontact> getAllContacts(){
        List<SQLcontact> contactList=new ArrayList<>();
        //select all query
